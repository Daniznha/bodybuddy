...
if (isset($_GET['delete_id'])) {
    ...
    $sql_delete = "DELETE FROM metas WHERE id_usuario = $delete_id";
    $conn->query($sql_delete);
    $sql_delete = "DELETE FROM treino WHERE id_usuario = $delete_id";
    $conn->query($sql_delete);
    $sql_delete = "DELETE FROM infos WHERE id_usuario = $delete_id";
    $conn->query($sql_delete);
    $sql_delete = "DELETE FROM usuarios WHERE id_usuario = $delete_id";
    $conn->query($sql_delete);
    header("Location: listaUsuarios.php"); // Redirecionar após excluir
    exit();
}
...
<body>
    ...
    if ($resultado->num_rows > 0) {
        ...
        echo "<thead><tr><th>ID</th><th>Nome</th><th>Email</th><th>Data de Nascimento</th><th>Sexo</th><th>Ações</th></tr></thead><tbody>";
